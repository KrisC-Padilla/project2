...
startBtn.addEventListener('click',function(){block.style.animation='slideDown 1s infinite';})
...
function moveRight(){
    ...
    moveSound.play();
    left +=100;
        if(left<=200){character.style.left = `${left}px`;}
}

//eventlistener to call both functions if arrow keys are pressed
document.addEventListener("keydown", function(event){
    if(event.key ==='ArrowLeft'){moveLeft();}
    else{(event.key ==='ArrowRight');moveRight();}
});

//to change the position of the block: can be 0px, 100px or 200px and initialize the score counter
    let scoreCounter=0; 
...
function howToAlertBox(){alert('Thank you for playing Block Oops! We hope you\'ll find it fun to play with. You can choose between three characters: AVA, IAN and KRIS. To play the game, use your arrow keys to move the character from left to right. Avoid being hit by DIBEL (the monster block in town!) to get a point. Hope you enjoy! Thank you.');}

//eventlistener for using touchscreen in mobile
document.getElementById("rightTouch").addEventListener("touchmove", moveRight);
document.getElementById("leftTouch").addEventListener("touchmove", moveLeft);
